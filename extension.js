...
function activate(context) {
  ...
  item = vscode.commands.registerCommand("text-to-translate.doTranslation", async (...commandArgs) => {
    ...
    const storeForTranslation = async (content) => {
      ...
      if (!workspaceFolders) return vscode.window.showErrorMessage('No workspace is open');
      ...
      if (!targetFile) return vscode.window.showErrorMessage('File i18n-translate-template not found in the workspace');
    ...
    const modifyFiles = (resources) => {
      ...
      if (resources === commandArgs[1] && holdSpace.length) storeForTranslation(holdSpace.join('\n'))
    }
